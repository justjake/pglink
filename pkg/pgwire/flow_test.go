...
func TestSimpleQueryFlow_BasicFlow(t *testing.T) {
	...
	flow := NewSimpleQueryFlow(
		ClientSimpleQueryQuery{T: &pgproto3.Query{String: "SELECT 1"}},
	...
	if !handlers.Update(ServerResponseDataRow{T: &pgproto3.DataRow{Values: [][]byte{[]byte("1")}}}) {
	...
	// More data rows
	handlers.Update(ServerResponseDataRow{T: &pgproto3.DataRow{Values: [][]byte{[]byte("2")}}})
	if flow.RowCount != 2 {
		t.Errorf("RowCount = %d, want 2", flow.RowCount)
	}

	// CommandComplete
	if !handlers.Update(ServerResponseCommandComplete{T: &pgproto3.CommandComplete{CommandTag: []byte("SELECT 2")}}) {
	...
	if handlers.Update(ServerResponseReadyForQuery{T: &pgproto3.ReadyForQuery{TxStatus: 'I'}}) {
...
func TestSimpleQueryFlow_WithError(t *testing.T) {
	flow := NewSimpleQueryFlow(
		ClientSimpleQueryQuery{T: &pgproto3.Query{String: "INVALID SQL"}},
		nil,
	)
	...
	// ErrorResponse
	if !handlers.Update(ServerResponseErrorResponse{T: &pgproto3.ErrorResponse{
		Severity: "ERROR",
		Code:     "42601",
...
func TestExtendedQueryFlow_ParseDescribeSync(t *testing.T) {
	...
	flow := NewExtendedQueryFlow(
		ClientExtendedQueryParse{T: &pgproto3.Parse{Name: "stmt1", Query: "SELECT $1"}},
	...
	// ParseComplete
	handlers.Update(ServerExtendedQueryParseComplete{T: &pgproto3.ParseComplete{}})

	// ParameterDescription
	handlers.Update(ServerExtendedQueryParameterDescription{T: &pgproto3.ParameterDescription{}})

	// RowDescription
	handlers.Update(ServerExtendedQueryRowDescription{T: &pgproto3.RowDescription{}})
...
func TestExtendedQueryFlow_ParseBindExecuteSync(t *testing.T) {
	flow := NewExtendedQueryFlow(
		ClientExtendedQueryParse{T: &pgproto3.Parse{Name: "", Query: "SELECT $1"}},
		nil,
	)
	...
	// Client: Bind
	handlers.Update(ClientExtendedQueryBind{T: &pgproto3.Bind{
	...
	// Client: Execute
	handlers.Update(ClientExtendedQueryExecute{T: &pgproto3.Execute{Portal: "p1"}})
	...
	handlers.Update(ServerExtendedQueryParseComplete{T: &pgproto3.ParseComplete{}})
	handlers.Update(ServerExtendedQueryBindComplete{T: &pgproto3.BindComplete{}})
	handlers.Update(ServerResponseDataRow{T: &pgproto3.DataRow{Values: [][]byte{[]byte("42")}}})
	if flow.RowCount != 1 {
		t.Errorf("RowCount = %d, want 1", flow.RowCount)
	}
	handlers.Update(ServerResponseCommandComplete{T: &pgproto3.CommandComplete{CommandTag: []byte("SELECT 1")}})
...
func TestExtendedQueryFlow_WithError(t *testing.T) {
	flow := NewExtendedQueryFlow(
		ClientExtendedQueryParse{T: &pgproto3.Parse{Name: "", Query: "INVALID"}},
		nil,
	)
	...
	handlers.Update(ServerResponseErrorResponse{T: &pgproto3.ErrorResponse{
		Severity: "ERROR",
		Code:     "42601",
		Message:  "syntax error",
	}})
...
func TestCopyFlow_CopyIn(t *testing.T) {
	...
	handlers.Update(ClientCopyCopyData{T: &pgproto3.CopyData{Data: []byte("line1\n")}})
	if flow.ByteCount != 6 {
		t.Errorf("ByteCount = %d, want 6", flow.ByteCount)
	}

	handlers.Update(ClientCopyCopyData{T: &pgproto3.CopyData{Data: []byte("line2\n")}})
	if flow.ByteCount != 12 {
		t.Errorf("ByteCount = %d, want 12", flow.ByteCount)
	}

	// Server: CommandComplete
	handlers.Update(ServerResponseCommandComplete{T: &pgproto3.CommandComplete{CommandTag: []byte("COPY 2")}})
...
func TestCopyFlow_CopyOut(t *testing.T) {
	...
	handlers.Update(ServerCopyCopyData{T: &pgproto3.CopyData{Data: []byte("row1\n")}})
	if flow.ByteCount != 5 {
		t.Errorf("ByteCount = %d, want 5", flow.ByteCount)
	}

	handlers.Update(ServerCopyCopyData{T: &pgproto3.CopyData{Data: []byte("row2\n")}})
...
func TestSimpleQueryRecognizer(t *testing.T) {
	...
	flow := handlers.Start(ClientSimpleQueryQuery{T: &pgproto3.Query{String: "SELECT 1"}})
	if flow == nil {
	...
	flow = handlers.Start(ClientExtendedQueryParse{T: &pgproto3.Parse{Query: "SELECT $1"}})
	if flow != nil {
...
func TestExtendedQueryRecognizer(t *testing.T) {
	...
	flow := handlers.Start(ClientExtendedQueryParse{T: &pgproto3.Parse{Name: "stmt1", Query: "SELECT $1"}})
	if flow == nil {
	...
	flow = handlers.Start(ClientExtendedQueryBind{T: &pgproto3.Bind{}})
	if flow != nil {
...
func TestCopyRecognizer(t *testing.T) {
	...
	flow := handlers.Start(ServerCopyCopyInResponse{T: &pgproto3.CopyInResponse{}})
	if flow == nil {
	...
	flow = handlers.Start(ServerCopyCopyOutResponse{T: &pgproto3.CopyOutResponse{}})
	if flow == nil {
	...
	flow = handlers.Start(ServerCopyCopyData{T: &pgproto3.CopyData{}})
	if flow != nil {
...
func TestProtocolState_FlowTracking(t *testing.T) {
	...
	queryMsg := ClientSimpleQueryQuery{T: &pgproto3.Query{String: "SELECT 1"}}
	...
	state.ProcessFlows(ServerResponseDataRow{T: &pgproto3.DataRow{}})
	state.ProcessFlows(ServerResponseCommandComplete{T: &pgproto3.CommandComplete{CommandTag: []byte("SELECT 1")}})
	...
	state.ProcessFlows(ServerResponseReadyForQuery{T: &pgproto3.ReadyForQuery{TxStatus: 'I'}})
...
func TestProtocolState_MultipleFlows(t *testing.T) {
	...
	state.ProcessFlows(ClientSimpleQueryQuery{T: &pgproto3.Query{String: "SELECT 1"}})
	...
	state.ProcessFlows(ClientExtendedQueryParse{T: &pgproto3.Parse{Name: "stmt1", Query: "SELECT $1"}})
	...
	state.ProcessFlows(ServerExtendedQueryParseComplete{T: &pgproto3.ParseComplete{}})
	state.ProcessFlows(ServerResponseReadyForQuery{T: &pgproto3.ReadyForQuery{TxStatus: 'I'}})
