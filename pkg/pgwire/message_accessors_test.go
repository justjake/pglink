...
func TestReadyForQuery_TxStatusByte(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			parsed := ServerResponseReadyForQuery(ServerParsed(&pgproto3.ReadyForQuery{TxStatus: tt.txStatus}))
			if got := parsed.TxStatusByte(); got != tt.txStatus {
				t.Errorf("TxStatusByte() from parsed = %c, want %c", got, tt.txStatus)
			}

			// Test with raw bytes
			raw := ServerResponseReadyForQuery(FromServer[*pgproto3.ReadyForQuery]{
...
func TestDataRow_ColumnCount(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			parsed := ServerResponseDataRow(ServerParsed(&pgproto3.DataRow{Values: values}))
			...
			raw := ServerResponseDataRow(FromServer[*pgproto3.DataRow]{
...
func TestCopyData_DataBytes(t *testing.T) {
	...
	serverParsed := ServerCopyCopyData(ServerParsed(&pgproto3.CopyData{Data: data}))
	...
	serverRaw := ServerCopyCopyData(FromServer[*pgproto3.CopyData]{
	...
	clientParsed := ClientCopyCopyData(ClientParsed(&pgproto3.CopyData{Data: data}))
	...
	clientRaw := ClientCopyCopyData(FromClient[*pgproto3.CopyData]{
		source: RawBody{Type: 'd', Body: data},
	})
...
func TestCopyData_DataSize(t *testing.T) {
	...
	server := ServerCopyCopyData(FromServer[*pgproto3.CopyData]{
		source: RawBody{Type: 'd', Body: data},
	})
	...
	client := ClientCopyCopyData(FromClient[*pgproto3.CopyData]{
		source: RawBody{Type: 'd', Body: data},
	})
...
func TestCommandComplete_CommandTagBytes(t *testing.T) {
	...
	// Test with parsed
	parsed := ServerResponseCommandComplete(ServerParsed(&pgproto3.CommandComplete{CommandTag: tag}))
	...
	raw := ServerResponseCommandComplete(FromServer[*pgproto3.CommandComplete]{
...
func TestCopyInResponse_Format(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Test with parsed
			parsed := ServerCopyCopyInResponse(ServerParsed(&pgproto3.CopyInResponse{OverallFormat: tt.format}))
			...
			raw := ServerCopyCopyInResponse(FromServer[*pgproto3.CopyInResponse]{
...
func TestCopyOutResponse_Format(t *testing.T) {
	// Test with parsed
	parsed := ServerCopyCopyOutResponse(ServerParsed(&pgproto3.CopyOutResponse{OverallFormat: 1}))
	if got := parsed.Format(); got != 1 {
		t.Errorf("Format() from parsed = %d, want 1", got)
	}

	// Test with raw
	raw := ServerCopyCopyOutResponse(FromServer[*pgproto3.CopyOutResponse]{
...
func TestRowDescription_FieldCount(t *testing.T) {
	// Test with parsed
	parsed := ServerExtendedQueryRowDescription(ServerParsed(&pgproto3.RowDescription{
	...
	raw := ServerExtendedQueryRowDescription(FromServer[*pgproto3.RowDescription]{
...
func TestParameterDescription_ParameterCount(t *testing.T) {
	// Test with parsed
	parsed := ServerExtendedQueryParameterDescription(ServerParsed(&pgproto3.ParameterDescription{
	...
	raw := ServerExtendedQueryParameterDescription(FromServer[*pgproto3.ParameterDescription]{
		source: RawBody{Type: 't', Body: body},
	})
...
func TestQuery_QueryBytes(t *testing.T) {
	...
	// Test with parsed
	parsed := ClientSimpleQueryQuery(ClientParsed(&pgproto3.Query{String: query}))
	...
	raw := ClientSimpleQueryQuery(FromClient[*pgproto3.Query]{
...
func TestParse_StatementNameBytes(t *testing.T) {
	// Test with parsed
	parsed := ClientExtendedQueryParse(ClientParsed(&pgproto3.Parse{Name: "stmt1", Query: "SELECT 1"}))
	...
	raw := ClientExtendedQueryParse(FromClient[*pgproto3.Parse]{
...
func TestBind_PortalNameBytes(t *testing.T) {
	// Test with parsed
	parsed := ClientExtendedQueryBind(ClientParsed(&pgproto3.Bind{
	...
	raw := ClientExtendedQueryBind(FromClient[*pgproto3.Bind]{
...
func TestExecute_PortalNameBytes(t *testing.T) {
	// Test with parsed
	parsed := ClientExtendedQueryExecute(ClientParsed(&pgproto3.Execute{Portal: "my_portal", MaxRows: 100}))
	...
	raw := ClientExtendedQueryExecute(FromClient[*pgproto3.Execute]{
...
func TestExecute_MaxRows(t *testing.T) {
	// Test with parsed
	parsed := ClientExtendedQueryExecute(ClientParsed(&pgproto3.Execute{Portal: "", MaxRows: 1000}))
...
func TestDescribe_ObjectType(t *testing.T) {
	...
	parsedStmt := ClientExtendedQueryDescribe(ClientParsed(&pgproto3.Describe{ObjectType: 'S', Name: "stmt1"}))
	if got := parsedStmt.ObjectType(); got != 'S' {
		t.Errorf("ObjectType() from parsed statement = %c, want S", got)
	}

	// Test portal describe with parsed
	parsedPortal := ClientExtendedQueryDescribe(ClientParsed(&pgproto3.Describe{ObjectType: 'P', Name: "portal1"}))
	...
	rawBody = append(rawBody, 0)
	raw := ClientExtendedQueryDescribe(FromClient[*pgproto3.Describe]{
...
func TestDescribe_ObjectNameBytes(t *testing.T) {
	// Test with parsed
	parsed := ClientExtendedQueryDescribe(ClientParsed(&pgproto3.Describe{ObjectType: 'S', Name: "test_stmt"}))
...
func TestClose_ObjectType(t *testing.T) {
	// Test with parsed
	parsed := ClientExtendedQueryClose(ClientParsed(&pgproto3.Close{ObjectType: 'S', Name: "stmt1"}))
	...
	rawBody = append(rawBody, 0)
	raw := ClientExtendedQueryClose(FromClient[*pgproto3.Close]{
		source: RawBody{Type: 'C', Body: rawBody},
	})
...
func TestClose_ObjectNameBytes(t *testing.T) {
	// Test with parsed
	parsed := ClientExtendedQueryClose(ClientParsed(&pgproto3.Close{ObjectType: 'S', Name: "to_close"}))
